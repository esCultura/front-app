...
export default function Categoria(props) {
  ...
  const seguir = async () => {
    ...
    simpleFetch(endpoint, "POST", { perfil: 6, tematica: props.tipus }).then(
      (data) => setData(data)
    );
    console.log("segueix");
  ...
  return (
    <View>
      <TouchableOpacity onPress={seguit ? deixarDeSeguir : seguir}>
        <Text style={seguit ? styles.type_unfollow : styles.type}>
          {props.tipus}
        </Text>
      </TouchableOpacity>
    </View>
